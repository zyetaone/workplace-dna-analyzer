...
import { d as db, a as attendees, s as sessions } from "../chunks/index2.js";
...
const loadAttendeeData = query(AttendeeDataSchema, async ({ sessionId, attendeeId }) => {
  const [session] = await db.select().from(sessions).where(eq(sessions.id, sessionId));
  if (!session) {
    error(404, "Session not found");
  }
  const [attendee] = await db.select().from(attendees).where(
    and(
...
const saveResponse = command(SaveResponseSchema, async ({ sessionId, attendeeId, questionIndex, response, generation }) => {
  try {
    const [currentAttendee] = await db.select().from(attendees).where(
      and(
        eq(attendees.id, attendeeId),
        eq(attendees.sessionId, sessionId)
      )
    ...
    await db.update(attendees).set(updateData).where(
      and(
        eq(attendees.id, attendeeId),
        eq(attendees.sessionId, sessionId)
      )
...
const completeQuiz = command(CompleteQuizSchema, async ({ sessionId, attendeeId, scores }) => {
  try {
    console.log("=== COMPLETING QUIZ ===");
    console.log("Session ID:", sessionId);
    console.log("Attendee ID:", attendeeId);
    console.log("Received scores:", JSON.stringify(scores));
    const updateResult = await db.update(attendees).set({
      completed: true,
    ...
    const [updated] = await db.select().from(attendees).where(eq(attendees.id, attendeeId));
...
const getProgress = query(AttendeeDataSchema, async ({ sessionId, attendeeId }) => {
  const [attendee] = await db.select().from(attendees).where(
    and(
      eq(attendees.id, attendeeId),
      eq(attendees.sessionId, sessionId)
