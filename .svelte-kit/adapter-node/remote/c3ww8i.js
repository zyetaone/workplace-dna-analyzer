...
const getSessionAnalytics = q(SessionIdSchema, async ({ sessionId }) => {
  const [session] = await d.select().from(s).where(eq(s.id, sessionId));
  if (!session) {
    error(404, "Session not found");
  }
  const sessionAttendees = await d.select().from(a).where(eq(a.sessionId, sessionId));
  ...
  if (completed.length > 0 && workplaceDNA) {
    ...
    const dominantGen = Object.entries(genDist).filter(([_, count]) => count > 0).sort(([, a2], [, b]) => b - a2)[0];
    if (dominantGen) {
...
const generateAIInsights = q(SessionIdSchema, async ({ sessionId }) => {
...
const endSession = a$1(EndSessionSchema, async ({ sessionId }) => {
  try {
    await d.update(s).set({
      isActive: false,
...
const subscribeToSession = q(SessionIdSchema, async ({ sessionId }) => {
  return {
...
const deleteAttendee = a$1(DeleteAttendeeSchema, async ({ sessionId, attendeeId }) => {
  try {
    const [attendee] = await d.select().from(a).where(eq(a.id, attendeeId));
    if (!attendee || attendee.sessionId !== sessionId) {
      error(404, "Attendee not found in this session");
    }
    await d.delete(a).where(eq(a.id, attendeeId));
...
for (const [name, fn] of Object.entries({ deleteAttendee, endSession, generateAIInsights, getSessionAnalytics, subscribeToSession })) {
  fn.__.id = "c3ww8i/" + name;
  fn.__.name = name;
}
export {
  deleteAttendee,
