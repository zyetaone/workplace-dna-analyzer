...
<div class="dna-profile-card {className}">
	<h3 class="mb-6 text-center text-xl font-semibold text-gray-900">{title}</h3>

	<!-- SVG Radar Chart -->
	<div class="relative mb-6 flex justify-center" in:scale={{ duration: 600, easing: quintOut }}>
		<svg width="300" height="300" class="transform">
			...
			<g class="opacity-10">
				{#each dimensions as [_, value], index}
					...
					<line x1="150" y1="150" {x2} {y2} stroke="currentColor" stroke-width="1" />
				{/each}
			</g>
	...
	<div class="space-y-3">
		{#each Object.entries(scores) as [trait, score], index}
			...
			<div
				class="flex items-center justify-between rounded-xl border border-gray-200 bg-gray-100 p-3"
			...
			>
				...
				<div class="flex items-center gap-3">
					...
					<span class="w-10 text-right text-sm font-bold text-gray-700">
						{score}%
					</span>
				</div>
			</div>
	...
	<!-- Insights -->
	<div
		class="mt-6 rounded-xl border border-purple-500/20 bg-gradient-to-r from-purple-500/10 to-pink-500/10 p-4"
	>
		<div class="flex items-start gap-3">
			<span class="text-2xl">💡</span>
			<div>
				<p class="mb-1 text-sm font-medium text-purple-300">Key Insight</p>
