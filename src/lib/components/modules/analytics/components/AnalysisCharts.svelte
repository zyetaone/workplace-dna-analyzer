<script lang="ts">
	...
	import { DonutChart, BarChart as D3BarChart, RadarChart as D3RadarChart } from '$lib/components/charts';
	// Local chart data helpers (inline to avoid dependencies)
	function createDonutData(data: Record<string, number>, colors: Record<string, string>) {
	...
	function createRadarData(scores: Record<string, number>, label: string) {
		return {
			label,
...
<div class="mb-8">
	...
	<div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
		...
		<Card variant="analytics" class="chart-entering stagger-1">
			{#snippet children()}
				<div class="chart-container">
					{#if generationDonutData()}
						<DonutChart data={generationDonutData()} variant="simple" />
					{:else}
						<div class="flex h-full flex-col items-center justify-center text-center">
							...
							<p class="text-sm text-gray-500">No participant data yet</p>
						</div>
					{/if}
				</div>
			{/snippet}
		...
		<Card variant="analytics" class="chart-entering stagger-2">
			{#snippet children()}
				<div class="chart-container">
					{#if averageScores()}
					...
					{:else}
						<div class="flex h-full flex-col items-center justify-center text-center">
							...
							<p class="text-sm text-gray-500">No completed responses yet</p>
						</div>
					{/if}
				</div>
			{/snippet}
		...
		<Card variant="analytics" class="chart-entering stagger-3">
			{#snippet children()}
				<div class="chart-container">
					{#if Object.keys(workplaceDnaData()).length > 0}
						<D3BarChart scores={workplaceDnaData()} />
					{:else}
						<div class="flex h-full flex-col items-center justify-center text-center">
							<svg
								class="mb-3 h-12 w-12 text-gray-400"
							...
							<p class="text-sm text-gray-500">Complete quiz to see DNA profile</p>
						</div>
					{/if}
				</div>
			{/snippet}
		...
		<Card variant="analytics" class="chart-entering stagger-4">
			{#snippet children()}
				<div class="chart-container">
					...
					{:else}
						<div class="flex h-full flex-col items-center justify-center text-center">
							...
							<p class="text-sm text-gray-500">Trends will appear as participants join</p>
						</div>
					{/if}
				</div>
			{/snippet}
