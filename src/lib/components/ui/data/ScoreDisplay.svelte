	...
	<div class="grid grid-cols-2 gap-4">
		{#each Object.entries(scores) as [key, value]}
			...
			>
				...
				{#if showValues}
					<div class="mb-2 text-2xl font-bold">{value}/{maxScore}</div>
				{/if}
				{#if showProgress}
					<div class="h-2 w-full rounded-full bg-gray-200">
						<div
							class="{progressClasses[
								config.color as keyof typeof progressClasses
							]} h-2 rounded-full {animated
								? 'delay-300 [transition:var(--transition-bounce)]'
								: ''}"
			...
			<div class="flex items-center gap-4">
				...
				<div class="h-3 flex-1 rounded-full bg-gray-200">
					<div
						class="{progressClasses[
							config.color as keyof typeof progressClasses
						]} h-3 rounded-full {animated ? '[transition:var(--transition-bounce)]' : ''}"
						style="width: {percentage}%"
					></div>
				</div>
				{#if showValues}
					<span class="w-12 font-mono text-sm">{value}/{maxScore}</span>
				{/if}
			</div>
		{/each}
	</div>
