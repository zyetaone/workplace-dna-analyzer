<script lang="ts">
		...
		'inline-flex items-center justify-center gap-2 font-medium rounded-lg transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed relative overflow-hidden',
		{
			variants: {
				variant: {
					...
					secondary:
						'bg-slate-700/60 backdrop-blur-sm text-slate-200 hover:bg-slate-600/70 focus-visible:ring-slate-400 border border-slate-600/50',
					destructive: 'bg-red-600 text-white hover:bg-red-700 focus-visible:ring-red-500',
					outline:
						'border border-slate-600 text-slate-300 hover:bg-slate-800/50 focus-visible:ring-slate-400',
					ghost: 'text-slate-300 hover:bg-slate-800/50 focus-visible:ring-slate-400',

					// Glass morphism variants
					glass:
						'bg-slate-900/40 backdrop-blur-xl text-slate-200 hover:bg-slate-800/60 focus-visible:ring-cyan-500 border border-slate-700/50',
					glassLight:
						'bg-white/10 backdrop-blur-sm text-white hover:bg-white/20 focus-visible:ring-white/50 border border-white/20',
	...
	interface Props
		extends Omit<HTMLButtonAttributes, 'onclick' | 'class' | 'type' | 'disabled'>,
			ButtonVariants {
		loading?: boolean;
		disabled?: boolean;
	...
	let buttonClass = $derived(cn(buttonVariants({ variant, size, fullWidth }), className));
...
>
	...
	{#if rippleEffect}
		<div
			...
			style="left: {rippleEffect.x - 10}px; top: {rippleEffect.y -
				10}px; width: 20px; height: 20px;"
			aria-hidden="true"
		></div>
	{/if}
