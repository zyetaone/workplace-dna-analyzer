/**
 * Shared Session Operations - Core server functions used across routes
 */
...
import { sessionCodeSchema, participantNameSchema, generationSchema, questionIndexSchema } from '$lib/utils/validation';
...
export const createSession = command(
  ...
  async ({ name }) => {
    ...
    // Generate unique enhanced code with retry logic
    do {
      code = generateSessionCode(name);
...
export const getSession = query(
  ...
  async ({ code }) => {
    ...
    if (!code || !/^[A-Z0-9]+-[0-9]{6}$/.test(code)) {
      throw new Error('Invalid session code format. Session codes must be in the format NAME-YYMMDD (e.g., TEAM-MEETING-241234).');
    }
