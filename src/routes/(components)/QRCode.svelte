<!-- Enhanced QR Code Component with {@attach...} and Modern Svelte 5 Patterns -->
<script lang="ts">
	...
	// QR Code generation with {@attach...} pattern and reactive $state
	function qrCodeAttachment(canvas: HTMLCanvasElement) {
		...
		async function generateQRCode() {
			if (!canvas || !qrData.url || isGenerating) return;
			...
			try {
				...
				const qr = QRCodeLib.create(qrData.url, {
					errorCorrectionLevel: 'M', // Medium error correction
					margin: 2,
					scale: Math.max(1, Math.floor(qrData.size / 25)) // Scale based on size
				});

				// Draw QR code on canvas
				const cellSize = qrData.size / qr.size;
				const margin = 2;
				...
				// Retry mechanism
				if (retryCount < qrData.retryAttempts) {
					retryCount++;
					console.log(`Retrying QR code generation (attempt ${retryCount}/${qrData.retryAttempts})`);
		...
		// Watch for reactive data changes and regenerate
		$effect(() => {
			// Track all reactive data changes
			qrData.url;
			qrData.size;
			qrData.retryAttempts;

			if (qrData.url && qrData.size) {
				generateQRCode();
			}
		});
