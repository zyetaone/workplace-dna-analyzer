<script lang="ts">
	...
	$effect(() => {
		if (typeof window !== 'undefined') {
			...
			if (hostname === 'localhost' || hostname === '127.0.0.1') {
				networkUrl = `${protocol}//${window.location.host}/session/${sessionId}/join`;
			} else {
				networkUrl = `${protocol}//${hostname}${port ? ':' + port : ''}/session/${sessionId}/join`;
			}
	...
	function drawWordCloud(words: Array<{text: string, size: number}>) {
		if (!words.length || !isClient) return;
		...
		function draw(cloudWords: any[]) {
			g.selectAll('text')
				...
				.style('fill', d => getConceptualColor(d.text))
	...
	// Update word cloud when data changes
	$effect(() => {
		if (isClient && analytics.wordCloudData?.length > 0) {
			setTimeout(() => {
				const container = document.getElementById('wordCloudContainer');
				if (container) {
					drawWordCloud(analytics.wordCloudData);
				}
			}, 200);
		}
	});
	...
	async function deleteParticipant(attendeeId: string, attendeeName: string) {
		...
		try {
			await deleteAttendee({ sessionId, attendeeId });
	...
	function copyParticipantLink(participantId: string) {
		const link = `${window.location.origin}/session/${sessionId}/attendee/${participantId}`;
...
>
		...
		{:else}
			...
			<div class="grid md:grid-cols-2 gap-8 mb-8">
					...
					{#if analytics.workplaceDNA}
						<div class="text-center py-8">
							...
							<div class="grid grid-cols-2 gap-4 mt-6">
								...
								<div class="bg-white rounded-lg p-4 shadow">
									<div class="text-3xl font-bold text-green-600">{analytics.preferenceScores.technology}</div>
				...
				<div class="bg-gradient-to-br from-white to-blue-50 rounded-lg shadow-lg p-8">
					...
					<div id="wordCloudContainer" style="height: 400px; width: 100%;"></div>
				</div>
			</div>
	...
	<!-- Unified AI Chatbot -->
	<UnifiedChatbot analytics={analytics} sessionId={sessionId} />
</div>